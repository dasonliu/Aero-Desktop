...
以下为 AI 生成并自动优化尺寸的核心逻辑：

```typescript
// 利用 Gemini 生成视觉资产并使用 Canvas 优化存储
const handleAiGenerate = async () => {
  ...
  const res = await ai.models.generateContent({
    ...
    contents: { parts: [{ text: "Japanese anime kanban girl icon, vibrant colors" }] },
    config: { imageConfig: { aspectRatio: '1:1' } }
  });

  const rawData = res.candidates?.[0]?.content?.parts.find(p => p.inlineData)?.inlineData?.data;

  // 图像压缩函数
  const optimized = await resizeImage(`data:image/png;base64,${rawData}`, 512, 512);
  
  // 更新状态
  setThumbnails(prev => ({ ...prev, icon: optimized }));
};
...
const resizeImage = (base64: string, w: number, h: number): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      resolve(canvas.toDataURL('image/webp', 0.6)); // 0.6 压缩率
    };
    img.src = base64;
  });
};
...
#AeroDesktop #GeminiAPI #React19 #FramerMotion #WebP压缩 #多态布局